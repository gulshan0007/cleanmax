...
def generate_proposal():
    ...
    try:
        ...
        if data.get("includeSavings", "").lower() == "yes":
            template_name = template_name.replace(",", "^J") + "-savings"

        doc = Document(f"proposal-temp-{template_name}.docx")
        ...
        for year in selected_years:
            tariff_key = f"tariff_{year}"
            tariff = float(data.get(tariff_key, "0"))
        ...
        if data.get("includeSavings", "").lower() == "yes":
            ...
            for year in selected_years:
                total_savings = 0
                tariff_key = f"tariff_{year}"
                solar_tariff = float(data.get(tariff_key, "0"))
                for i in range(year):
